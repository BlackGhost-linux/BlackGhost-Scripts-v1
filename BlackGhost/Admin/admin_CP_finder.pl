...
system('cls');
system('title Admin CP Finder (MagicCoder Edition)');
system ("color 0a");
print"\n";
print "\t>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>\n";
print "\t>   Admin CP Finder (MagicCoder Edition)  >\n";
print "\t>     Edited By Mr.38 From INDISHELL      >\n";
print "\t>  Greetz: Ashell_INDIA | rOot_devil      >\n";
print "\t>           LUCKY       | Cod3d32         >\n";
print "\t> more >> hackforsecurity.blogspot.com    >\n";
print "\t>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>\n";
print "\n";
